...
export const MODULE_ACTIONS = {
    ...
    [MODULES.ANALYTICS]: [],
    [MODULES.SETTINGS]: [ACTIONS.EDIT, ACTIONS.VIEW_HISTORY],
    [MODULES.GENERAL]: [ACTIONS.EDIT, ACTIONS.VIEW_HISTORY],
    [MODULES.HOME]: [ACTIONS.EDIT, ACTIONS.VIEW_HISTORY],
    [MODULES.ABOUT]: [ACTIONS.EDIT, ACTIONS.VIEW_HISTORY],
    [MODULES.CONTACT]: [ACTIONS.EDIT, ACTIONS.VIEW_HISTORY],
    [MODULES.MESSAGES]: [ACTIONS.DELETE, ACTIONS.VIEW_HISTORY],
    [MODULES.PROFILE]: [ACTIONS.EDIT],
    [MODULES.AUDIT]: []
};
...
const isDefaultRoleCapabilityAllowed = (action, moduleName, normalizedRole) => {
    ...
    if (normalizedRole === 'editor') {
        // Editors are restricted from Admin/SuperAdmin/System data layers
        const adminModules = [MODULES.DATABASE, MODULES.MESSAGES, MODULES.AUDIT, MODULES.SETTINGS, MODULES.ANALYTICS];
        if (adminModules.includes(moduleName)) return false;

        // Default modules allowed for editor with full permissions
        const editorModules = [MODULES.BLOG, MODULES.PROJECTS];
        
        if (editorModules.includes(moduleName)) {
            const superAdminOnlyActionsForEditor = [ACTIONS.DATABASE_ACTIONS];
            return !superAdminOnlyActionsForEditor.includes(action);
        }
        
        // Other tabs (like EXPERIENCE) must be explicitly enabled for editors.
        // View is the only default if enabled but not configured.
...
export const isActionAllowed = (action, moduleName, role, rolePermissions = {}) => {
    ...
    if (isRoleConfigured) {
        ...
        if (capabilityRole === 'admin') {
            // Admins inherit full power for any enabled tab (except system-level modules)
            const superAdminOnlyActions = [ACTIONS.DATABASE_ACTIONS];
            if (superAdminOnlyActions.includes(action)) return false;
            return true;
        }

        if (capabilityRole === 'editor') {
            // Editors ONLY inherit basic CRUD: View, Create, Edit.
            // High-privilege actions (Delete, Feature, Visibility) MUST be explicit for custom roles.
            const safeEditorActions = [ACTIONS.VIEW, ACTIONS.CREATE, ACTIONS.EDIT, ACTIONS.VIEW_HISTORY];
            return safeEditorActions.includes(action);
        }
