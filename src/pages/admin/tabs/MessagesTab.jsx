...
const MessagesTab = ({ userRole, showToast, isActionAllowed, onMessagesChange }) => {
    ...
    const pagination = useCursorPagination(messagesPerPage, [debouncedSearch]);

    // 1. Fetch Paginated Messages
    const { data: messagesResult = { data: [], lastDoc: null, hasMore: false }, isLoading, isFetching } = useQuery({
        queryKey: ['messages', debouncedSearch, pagination.cursor, messagesPerPage],
        queryFn: async () => {
            const result = await MessageService.fetchMessagesPaginated({
    ...
    return (
        <div style={{ paddingBottom: '3rem' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
                <div className={styles.searchBox} style={{ margin: 0, flex: 1, minWidth: '250px', position: 'relative' }}>
                    ...
                    />
                    {(isLoading || isFetching) && <RefreshCw size={14} className={styles.spin} style={{ position: 'absolute', right: '1rem', top: '50%', transform: 'translateY(-50%)', opacity: 0.5 }} />}
                    {debouncedSearch && messages.length > 0 && <span className={styles.searchResultCount} style={{ right: (isLoading || isFetching) ? '2.5rem' : '1rem' }}>{messages.length} results on this page</span>}
                </div>
