...
import { normalizeSpeaking, validateSpeaking } from '../domain/speaking/speakingDomain';
import { getLanguageFromStorage, localizeEntityFields } from '../utils/localization';

const SPEAKING_LOCALIZED_FIELDS = ['title', 'eventName', 'location', 'description'];
...
class SpeakingService extends BaseService {
    ...
    localizeSpeaking(speaking, lang = getLanguageFromStorage()) {
        return localizeEntityFields(speaking, SPEAKING_LOCALIZED_FIELDS, lang);
    }
    ...
    async deleteSpeaking(userRole, id, trackDelete) {
        return this.delete(id, trackDelete);
    }

    async batchDeleteSpeaking(userRole, ids, trackDelete) {
        return this.batchDelete(ids, trackDelete);
    }

    async batchUpdateSpeakingVisibility(userRole, ids, visible, trackWrite) {
        return this.batchUpdate(ids, { visible }, trackWrite);
    }

    async batchUpdateSpeakingFeatured(userRole, ids, featured, trackWrite) {
        return this.batchUpdate(ids, { featured }, trackWrite);
    }

    async saveSpeaking(userRole, formData, trackWrite) {
        const dataToSave = normalizeSpeaking(formData);
        const errors = validateSpeaking(dataToSave);
        if (errors) {
